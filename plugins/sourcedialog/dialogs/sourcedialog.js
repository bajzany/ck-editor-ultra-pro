...
CKEDITOR.dialog.add( 'sourcedialog', function( editor ) {
	...
	return {
		...
		onShow: function() {
                        var area = $(this.getElement().$).find('.aceditor');
                        //area.hide();
                        area.parents('.cke_reset_all').removeClass("cke_reset_all");
			this.setValueOf( 'main', 'data', oldData = editor.getData());
                        area.hide();
                        
                        if(acee == false){
                            div = $('<div id="aceeditor" style="bottom: 50px;left: 5px; right: 5px; position: absolute; top: 32px;"></div>');
                            area.parent().append(div);

                            acee = ace.edit(div[0]);
                            acee.setTheme("ace/theme/github");
                            acee.getSession().setMode("ace/mode/html");
                        }
                        
                        acee.getSession().setValue(editor.getData());
		},
		...
		onOk: ( function() {
			...
			return function() {
				...
				var newData =  acee.getValue();//this.getValueOf( 'main', 'data' ).replace( /\r/g, '' ),
					that = this;
